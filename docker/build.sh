...
cd ~/taiwan-topo
./tools/docker-make distclean-extracts
git clean -fd
git checkout master -- .
git pull --rebase
...
if [[ "${TARGET}" == *"suites"* ]]; then
    ./tools/docker-make INSTALL_DIR=/workspace/${INSTALL_DIR} ${TARGET}
    ./tools/docker-make exps || echo make exps failed
    cd ${INSTALL_DIR}
